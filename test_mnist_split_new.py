...
Settings:
  ...
  --name_style multilang  party 0 English / 1 Chinese / 2 Spanish descriptions
  --desc_encoder st:paraphrase-multilingual-MiniLM-L12-v2   semantic description encoder for psi_filter (pip install sentence-transformers);
                         default 'char' = character n-grams, which CANNOT match "0" with "zero"
  psi_attn / psi_attn_filter  descriptions -> --text_encoder (sentence-transformers, default paraphrase-multilingual-MiniLM-L12-v2)
                         -> attention over public TEXT anchors (rt_protocol.TEXT_ANCHORS, no number words)
                         -> corrected log-attention coordinates -> confident fuzzy PSI matching.
                         attn_filter fuses text/image ranks before matching, requiring strong evidence.
  --attn_coord alpha --attn_match intersection --attn_confidence positive reproduces the previous version.
  --descriptions short|keywords|long   single keyword or [domain, class] keywords;
                         long is a compatibility alias for keywords (the default).

  python3 test_mnist_split_new.py --device mps --seeds 5
  python3 test_mnist_split_new.py --device mps --seeds 5 --per_class 20
  python3 test_mnist_split_new.py --device mps --seeds 5 --name_style multilang --descriptions long --desc_encoder st:paraphrase-multilingual-MiniLM-L12-v2
...
# multilang: party k uses LANGS[k % 3]  (class identity and domain/context keyword)
LANGS = [
    ("en", ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"],
     ["digit", "{}"]),
    ("zh", ["零", "一", "二", "三", "四", "五", "六", "七", "八", "九"], ["數字", "{}"]),
...
def parse():
    ...
    ap.add_argument("--text_encoder", default=MULTILINGUAL, help="sentence-transformers model for psi_attn* (multilingual)")
    ...
    ap.add_argument("--descriptions", default="keywords", choices=["short", "keywords", "long"])
    ap.add_argument("--attn_coord", default="log_whiten", choices=["alpha", "pooled", "log_whiten"])
    ap.add_argument("--attn_heads", type=int, default=1)
    ap.add_argument("--attn_match", default="precision", choices=["precision", "fusion", "intersection"],
                    help="attn_filter: offline verification, previous rank fusion, or original intersection")
    ap.add_argument("--verify_floor", type=float, default=.90)
...
def describe(name, how):
    """Class semantic metadata; each item is encoded independently."""
    if how == "short":
        return [name]
    if how not in ("keywords", "long"):
        raise ValueError(f"unknown description style: {how}")
    for _, words, template in LANGS[1:]:
        if name in words:
            return [keyword.format(name) for keyword in template]
    return [keyword.format(name) for keyword in LANGS[0][2]]


def encode_keywords(encode, descriptions):
    """Batch unique keywords without pooling context and class before attention."""
    vocabulary = list(dict.fromkeys(k for description in descriptions for k in description))
    vectors = dict(zip(vocabulary, encode(vocabulary)))
    return [np.stack([vectors[k] for k in description]) for description in descriptions]
...
def desc_vectors(a, names):
    ...
    # Non-attention baseline: mean independently encoded keyword vectors.
    vectors = encode_keywords(lambda texts: _CACHE["st"].encode(texts, normalize_embeddings=True),
                              [describe(n, a.descriptions) for n in names])
    return np.stack([v.mean(axis=0) for v in vectors])
...
def psi_inputs(a, parties, dev, seed, need_text):
    ...
    def emb(x):
        ...
        for batch in x.split(64):
            batch = F.interpolate(batch.to(dev) * 0.5 + 0.5, size=64, mode="bilinear", align_corners=False)
            batches.append(enc((batch - _MEAN.to(dev)) / _STD.to(dev)).cpu().numpy())
    ...
    for p in parties:
        ...
        for c in p["clients"]:
            features = {l: emb(torch.stack([x for x, y in c["data"] if y == l]))
                        for l in range(len(p["digits"]))}
            summ = {l: f[:64].mean(0) - mu for l, f in features.items()}
            clients[c["cid"]] = {"summ": summ, "names": dict(enumerate(p["names"])),
                                 "precision_summ": {l: f.mean(0) - mu for l, f in features.items()},
                                 "count": {l: len(f) for l, f in features.items()}}
            if dv is not None:
    ...
    if need_text:                                   # attn*: every client's enc_i = the public text encoder
        ...
        for p in parties:
            descriptions = [describe(n, a.descriptions) for n in p["names"]]
            tv = encode_keywords(lambda texts: text_vectors(a, texts), descriptions)
            for c in p["clients"]:
                clients[c["cid"]]["keyword_vecs"] = dict(enumerate(tv))
                clients[c["cid"]]["keywords"] = dict(enumerate(descriptions))
        ...
        if method.startswith("psi_"):
            ...
            if "review_pairs" in diagnostics:
                with open(os.path.join(out, f"review_{method}_seed{seed}.csv"), "w", newline="") as f:
                    ...
                    for i, x, j, y, reason in diagnostics["review_pairs"]:
                        writer.writerow([i, x, j, y, reasons[int(reason)]])
