...
def run_offline(args):
    ...
    for rnd in rounds:
        ...
        for threshold in thresholds:
            ...
            elif args.label_mapping == "psi_trivial":
                # exact DH-PSI on shared class names (label_mapping/psi_trivial_new.py).
                # Uses no GANs / client models, so the table is the same for every round.
                psi_clients, group_of = {}, {}
                for cid, info in client_metadata.items():
                    if info["group_name"] not in active_datasets:
                        continue
                    psi_clients[cid] = {"names": {a: info["class_names"][a] for a in info["valid_labels"]},
                                        "count": {a: 1 for a in info["valid_labels"]}}
                    group_of[cid] = info["group_name"]
                table, _, _ = run_trivial(psi_clients, psi="dh", seed=args.seed or 0, log=logger.log)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--label_mapping", type=str, default="improve_single", 
                        choices=["image-bi", "image-single", "missing_link", "missing_link_single", "improve_single", "image-cs", "improve_single_noniid", "temp", "temp2", "feature", "psi_trivial"])
